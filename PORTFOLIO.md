...
It is designed to answer a practical sequence of operational questions:
...
- **Conformalized Quantile Regression (CQR)** for finite-sample uncertainty intervals;
- **SHAP** for local predictive explanations;
- explicit frozen thresholds and model-registry contracts.

## Research validation

The repository includes a separated research workflow with frozen contracts, temporal folds, experiment runners, metrics, figures, and manuscript evidence traceability.

The primary research evidence uses **five expanding-origin temporal development folds** with 90-day embargoes (`N = 7,306` shipments; `1,125` delays). A later `N = 1,013` shipment cohort with `61` delays is treated only as a **secondary locked replication benchmark**, because it had already been evaluated historically by the serving registry.
...
| Nominal 90% CQR empirical coverage | **91.8% (56/61)** |
...
- research-specific quarantine, temporal-fold, artifact, and manifest integrity tests;
...
- [`docs/REPRODUCIBILITY.md`](docs/REPRODUCIBILITY.md) — serving-registry reproducibility
