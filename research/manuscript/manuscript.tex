...
\title{\textbf{Calibrated Delay Risk and Conformal Severity Prediction under Temporal Distribution Shift in Pharmaceutical Supply Chains}}

\author{
  ORCA Research Engineering Team\\
  \texttt{research@orca-control-tower.local}\\
  Logistics Decision Intelligence Laboratory
}
...
To address these challenges systematically, we present the \textbf{ORCA} (Operational Risk \& Conformal Analytics) research framework. ORCA is an integrated, four-stage decision-intelligence framework designed for mission-critical logistics environments.
...
All features $\mathbf{x}_i \in \mathcal{X}$ are restricted to information known on or before $T_{\text{pred}, i}$. The feature space comprises 39 operational variables (13 categorical and 26 continuous variables). All post-delivery operational variables---including actual delivery dates and delay outcomes---are strictly excluded from $\mathcal{X}$.
...
\section{Primary Development Results}
...
\section{Secondary Locked Registry Benchmark}
Table \ref{tab:locked_benchmark} presents classification performance on the secondary Locked Registry Evaluation Set ($N = 1,013$, 61 delays, prevalence $6.02\%$).

\begin{table}[htbp]
\centering
\caption{Secondary locked registry benchmark classification results.}
\label{tab:locked_benchmark}
\small
\begin{tabular}{llcccccccc}
\toprule
\textbf{Model} & \textbf{Role} & \textbf{Calib} & \textbf{Threshold} & \textbf{PR-AUC} & \textbf{ROC-AUC} & \textbf{Brier} & \textbf{ECE} & \textbf{Recall} & \textbf{F1} \\
\midrule
CatBoost & Primary Model & Platt & 0.1000 & 0.2709 & 0.7477 & 0.0527 & 0.0568 & 0.7377 (45/61) & \textbf{0.3358} \\
Random Forest & Sensitivity & Platt & 0.1050 & \textbf{0.3195} & \textbf{0.7898} & \textbf{0.0493} & \textbf{0.0314} & \textbf{0.7705 (47/61)} & 0.3322 \\
\bottomrule
\end{tabular}
\end{table}

Random Forest achieved higher PR-AUC ($0.3195$ vs. $0.2709$) and ROC-AUC ($0.7898$ vs. $0.7477$), while CatBoost achieved a comparable F1 score ($0.3358$ vs. $0.3322$). 

On the 61 delayed shipments, Conditional Median achieved an MAE of $9.05$ days (MedAE: $7.00$d). CQR achieved empirical coverage of **$70.49\%$** at 80\% nominal ($43/61$, CI: $[57.43\%, 81.48\%]$, width: $41.04$d), **$91.80\%$** at 90\% nominal ($56/61$, CI: $[81.90\%, 97.28\%]$, width: $46.27$d), and **$100.00\%$** at 95\% nominal ($61/61$, CI: $[94.13\%, 100.00\%]$, width: $61.74$d) \cite{clopper1934use}.

\section{Operational Capacity-Constrained Prioritization [SIMULATED SCENARIO]}
Table \ref{tab:decision_utility} summarizes operational triage performance across inspection capacities $K \in \{1\%, 5\%, 10\%, 20\%\}$ on the locked benchmark.
